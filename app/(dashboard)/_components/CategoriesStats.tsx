...
import { getCategoriesStatResponseType } from '@/app/api/stats/categories/route';
...
function CategoriesStats({ userSettings, from, to }: Props) {
  const statsQuery = useQuery<getCategoriesStatResponseType>({
...
function CategoriesCard({ data, type, formatter }: { type: TransactionType; formatter: Intl.NumberFormat; data: getCategoriesStatResponseType }) {
  ...
  return (
    <Card className="h-80 w-full col-span-6">
      <CardHeader>
        <CardTitle className="grid grid-flow-row justify-between gap-2 text-muted-foreground md:grid-flow-col">{type === 'income' ? 'Income' : 'Expense'} by category</CardTitle>
      </CardHeader>
